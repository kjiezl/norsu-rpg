...
CBA::CBA(RenderWindow& window) : window(window), fadeTrans(window), dPressed(false),
aPressed(true), dampingFactor(0.3f), pauseMenu(window), gamePaused(false),
dialogueBox(window), goingHM(false), startLevel2(false), startLevel3(false),
...
showGhost14(true), showScores(true), bossDefeated(false), goingRoom(false), 
healthCount(window), weapon1Show(false), weapon2Show(false), weapon3Show(false),
inv(window), start5(false), start6(false), start7(false), start8(false), start9(false), start10(false),
...
slowed15(false), start1(false), start2(false), start3(false), start4(false),
slowed1(false), slowed2(false), slowed3(false), slowed4(false), hp(window, 100.0),
showHP(true){
	...
	bgTexture.loadFromFile("sprites\\maps\\cba-night.png");
...
Vector2f lerp4(const Vector2f& start, const Vector2f& end, float t) {
...
void CBA::initPlayer() {
	...
	task3.setString("!\tGo inside the room");
	...
	// big lower 1
	rectangles.push_back(Rectangle(200, 455, 1040, 600, rectAlpha));
	rectangles.push_back(Rectangle(1473, 455, 1200, 600, rectAlpha)); // lower 2
...
void CBA::showTask2() {
	task2.setString("!\tDefeat the boss");
...
void CBA::handleInput() {
	...
	if (Keyboard::isKeyPressed(Keyboard::Tab) && levelComplete3) {
		bossDefeated = true;
	}

	// 600 470 - 750 550
	if (levelComplete3 && playerSprite.getPosition().x <= 300 && playerSprite.getPosition().x >= -20 && playerSprite.getPosition().y <= 440 && playerSprite.getPosition().y >= 100) {
		goingHM = true;
	}

	// 1757 155
	if (bossDefeated && playerSprite.getPosition().x <= 1770 && playerSprite.getPosition().x >= 1740 && playerSprite.getPosition().y <= 170 && playerSprite.getPosition().y >= 90 && levelComplete3 && Keyboard::isKeyPressed(Keyboard::E)) {
		goingRoom = true;
	}
...
void CBA::updateTextPos() {
	...
	task3.setPosition(camera.getCenter().x - window.getSize().x / 2.6f + 900, camera.getCenter().y - window.getSize().y / 1.80f);
...
void CBA::Camera() {
	...
	newCenter = lerp4(currentCenter, targetCenter, dampingFactor);
...
void CBA::update() {
	...
	if (!slowed1) {
		c1.restart();
		speed1 = 1.5;
		ghost1Sprite.setColor(Color(255, 255, 255, 255));
	}
	else {
		speed1 = 1.0;
		ghost1Sprite.setColor(Color(85, 160, 255, 200));
	}

	float e1 = c1.getElapsedTime().asSeconds();
	...
	float elapsedTime1 = clock1.getElapsedTime().asSeconds();
	...
	if (!slowed2) {
		c2.restart();
		speed2 = 1.5;
		ghost2Sprite.setColor(Color(255, 255, 255, 255));
	}
	else {
		speed2 = 1.0;
		ghost2Sprite.setColor(Color(85, 160, 255, 200));
	}

	float e2 = c2.getElapsedTime().asSeconds();
	...
	float elapsedTime2 = clock2.getElapsedTime().asSeconds();
	...
	if (!slowed3) {
		c3.restart();
		speed3 = 1.5;
		ghost3Sprite.setColor(Color(255, 255, 255, 255));
	}
	else {
		speed3 = 1.0;
		ghost3Sprite.setColor(Color(85, 160, 255, 200));
	}

	float e3 = c3.getElapsedTime().asSeconds();
	...
	float elapsedTime3 = clock3.getElapsedTime().asSeconds();
	...
	if (!slowed4) {
		c4.restart();
		speed4 = 1.5;
		ghost4Sprite.setColor(Color(255, 255, 255, 255));
	}
	else {
		speed4 = 1.0;
		ghost4Sprite.setColor(Color(85, 160, 255, 200));
	}

	float e4 = c4.getElapsedTime().asSeconds();
	...
	float elapsedTime4 = clock4.getElapsedTime().asSeconds();
...
void CBA::playerPosFromHM() {
	playerSprite.setPosition(360, 230);
}

void CBA::goingHMFalse() {
	goingHM = false;
}
...
void CBA::playerPosFromRoom() {
	playerSprite.setPosition(2525, 160);
}

void CBA::goingRoomFalse() {
	goingRoom = false;
}

bool CBA::isIntersecting(const FloatRect& rect1, const FloatRect& rect2) {
	return rect1.intersects(rect2);
}

void CBA::handleCollision(const Rectangle& rect) {
